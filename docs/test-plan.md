...
- 需要用户在 SillyTavern 中进行长期或手工验证时，必须构建并提交仓库根目录 `index.js`。
- 测试提交必须推送到 `hajimi-kun/shujuku`，确保远端提交包含本次源码、测试和构建产物。
- 最终回复必须提供固定到完整提交哈希的 jsDelivr 链接，格式为：
  `https://cdn.jsdelivr.net/gh/hajimi-kun/shujuku@<完整提交哈希>/index.js`
- 不使用分支名或短哈希作为长期测试链接，避免后续提交或缓存导致测试版本漂移。
